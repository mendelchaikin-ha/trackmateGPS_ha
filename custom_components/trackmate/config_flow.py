...
class TrackmateConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            try:
                _LOGGER.info(
                    "Trackmate: validating connection to %s for user %s",
                    user_input[CONF_FLARESOLVERR_URL], username,
                )
                await client.validate()
                _LOGGER.info("Trackmate: validation successful")
            ...
            except TrackmateConnectionError as err:
                _LOGGER.error("Trackmate: connection error: %s", err)
                errors["base"] = "cannot_connect"
            except TrackmateAuthError as err:
                _LOGGER.error("Trackmate: auth error: %s", err)
                errors["base"] = "invalid_auth"
            except Exception:
                _LOGGER.exception("Trackmate: unexpected error during setup")
                errors["base"] = "unknown"
            finally:
                await client.close()
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(
                        CONF_FLARESOLVERR_URL, default=DEFAULT_FS_URL
                    ): str,
...
class TrackmateOptionsFlow(OptionsFlow):
    ...
    async def async_step_init(
        ...
        try:
            client = TrackmateClient(
                fs_url=self.config_entry.data[CONF_FLARESOLVERR_URL],
        ...
        if vehicle_opts:
            schema[
                vol.Optional(
                    CONF_VEHICLE_IDS, default=cur.get(CONF_VEHICLE_IDS, [])
                )
            ] = cv.multi_select(vehicle_opts)
